// 功能支持
import { app, ipcMain, Menu, globalShortcut, nativeTheme } from 'electron';
import { SETTING_NAME, BASE_SETTING, UPDATE_URL, UPDATE_MESSAGE } from '@constants/common';
...
export const AddAppSetting = () => {
    let userSetting = systemSetting.get(SETTING_NAME);
    if (!userSetting) {
        userSetting = {
            ...BASE_SETTING,
            openAtLogin: app.getLoginItemSettings().openAtLogin || false,
        };
        systemSetting.set(SETTING_NAME, userSetting);
    }
    dealUserSetting({...BASE_SETTING, ...userSetting });
    // 获取设置
    mainEvent(ACTION_KEY.getSetting, sender => {
        const setting = systemSetting.get(SETTING_NAME);
        sender(setting);
    });
    // 修改设置
    mainEvent(ACTION_KEY.setSetting, (sender, params = {}) => {
        ...
        // 清除一下
        systemSetting.set(SETTING_NAME, _setting);
...
export const AddDataBase = (name) => {
    ...
    mainEvent(ACTION_KEY.dbFind(name), (sender, condition = {}) => {
        db.find({ ...condition }).sort({ date: -1 }).exec((err, docs) => {
...
export const DeleteShortcuts = (key) => {};

export const AddDarkMode = (callback) => {
...
export const AddSearchUpdate = (win) => {
    mainEvent(ACTION_KEY.searchAction, (sender, info) => {
        const { height } = info;
